...
PointHessian* FullSystem::optimizeImmaturePoint(ImmaturePoint* point,
        int minObs, ImmaturePointTemporaryResidual* residuals)
{
    int nres = 0;
            ...
            if (r->target == frameHessians.back())
            {
                ...
                p->lastResiduals[0].second = ResState::IN;
                    ...
                    == (frameHessians.size() < 2 ?
                            0 : frameHessians[frameHessians.size() - 2]))
            {
