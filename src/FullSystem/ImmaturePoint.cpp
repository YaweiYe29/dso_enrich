...
ImmaturePoint::ImmaturePoint(int u_, int v_, FrameHessian* host_, float type,
        CalibHessian* HCalib) :
        u(u_), v(v_), host(host_), my_type(type), idepth_min(0), idepth_max(
                NAN), lastTraceStatus(IPS_UNINITIALIZED)
{
...
ImmaturePointStatus ImmaturePoint::traceOn(FrameHessian* frame,
        Mat33f hostToFrame_KRKi, Vec3f hostToFrame_Kt, Vec2f hostToFrame_affine,
        CalibHessian* HCalib, bool debugPrint)
{
    ...
    if (debugPrint)
        printf(
                "trace pt (%.1f %.1f) from frame %d to %d. Range %f -> %f. t %f %f %f!\n",
...
//	const float minImprovementFactor = 2;		// if pixel-interval is smaller than this, leave it be.
    ...
    if (debugPrint)
        printf(
                "trace pt (%.1f %.1f) from frame %d to %d. Range %f (%.1f %.1f) -> %f (%.1f %.1f)! ErrorInPixel %.1f!\n",
    ...
    for (int it = 0; it < setting_trace_GNIterations; it++)
    {
        ...
        for (int idx = 0; idx < patternNum; idx++)
        {
            ...
            float residual = hitColor[0]
                    - (hostToFrame_affine[0] * color[idx]
                            + hostToFrame_affine[1]);
        ...
        if (energy > bestEnergy)
        {
            ...
            if (debugPrint)
                printf(
                        "GN BACK %d: E %f, H %f, b %f. id-step %f. UV %f %f -> %f %f.\n",
        ...
        } else
        {
            ...
            if (debugPrint)
                printf(
                        "GN step %d: E %f, H %f, b %f. id-step %f. UV %f %f -> %f %f.\n",
...
float ImmaturePoint::calcResidual(CalibHessian * HCalib,
        const float outlierTHSlack, ImmaturePointTemporaryResidual* tmpRes,
        float idepth)
{
    FrameFramePrecalc* precalc = &(host->targetPrecalc[tmpRes->target->idx]);
    ...
    for (int idx = 0; idx < patternNum; idx++)
    {
        ...
        float hw =
                fabsf(residual) < setting_huberTH ?
                        1 : setting_huberTH / fabsf(residual);
        energyLeft += weights[idx] * weights[idx] * hw * residual * residual
                * (2 - hw);
    }
