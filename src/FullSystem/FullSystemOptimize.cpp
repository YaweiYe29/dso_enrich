...
void FullSystem::linearizeAll_Reductor(bool fixLinearization,
        std::vector<PointFrameResidual*>* toRemove, int min, int max,
        Vec10* stats, int tid)
{
...
void FullSystem::setNewFrameEnergyTH()
{
    ...
    newFrame->frameEnergyTH *= setting_overallEnergyTHWeight
            * setting_overallEnergyTHWeight;
    ...
    if (multiThreading)
    {
        treadReduce.reduce(
                boost::bind(&FullSystem::linearizeAll_Reductor, this,
                        fixLinearization, toRemove, _1, _2, _3, _4), 0,
...
void FullSystem::printOptRes(Vec3 res, double resL, double resM,
...
float FullSystem::optimize(int mnumOptIts)
{
    ...
    if (multiThreading)
        treadReduce.reduce(
                boost::bind(&FullSystem::applyRes_Reductor, this, true, _1, _2,
                        _3, _4), 0, activeResiduals.size(), 50);
    else
    ...
    for (int iteration = 0; iteration < mnumOptIts; iteration++)
    {
        ...
        bool canbreak = doStepFromBackup(stepsize, stepsize, stepsize, stepsize,
                stepsize);
    ...
    newStateZero.segment < 2 > (6) = frameHessians.back()->get_state().segment
            < 2 > (6);
    ...
    statistics_lastFineTrackRMSE = sqrtf(
            (float) (lastEnergy[0] / (patternNum * ef->resInA)));
...
void FullSystem::solveSystem(int iteration, double lambda)
{
    ef->lastNullspaces_forLogging = getNullspaces(ef->lastNullspaces_pose,
            ef->lastNullspaces_scale, ef->lastNullspaces_affA,
...
std::vector<VecX> FullSystem::getNullspaces(std::vector<VecX> &nullspaces_pose,
        std::vector<VecX> &nullspaces_scale, std::vector<VecX> &nullspaces_affA,
