...
void FrameHessian::setStateZero(Vec10 state_zero)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        SE3 w2c_leftEps_M_x0 = (get_worldToCam_evalPT() * EepsM)
                * get_worldToCam_evalPT().inverse();
        nullspaces_pose.col(i) = (w2c_leftEps_P_x0.log()
                - w2c_leftEps_M_x0.log()) / (2e-3);
    }
    ...
    nullspaces_scale = (w2c_leftEps_P_x0.log() - w2c_leftEps_M_x0.log())
            / (2e-3);
        ...
        CalibHessian* HCalib)
{
    ...
    PRE_aff_mode =
            AffLight::fromToVecExposure(host->ab_exposure, target->ab_exposure,
                    host->aff_g2l(), target->aff_g2l()).cast<float>();
