        ...
        for (int y = 0; y < hl; y++)
            for (int x = 0; x < wl; x++)
            {
                ...
                idepth_l[x + y * wl] = idepth_lm[bidx] + idepth_lm[bidx + 1]
                        + idepth_lm[bidx + wlm1] + idepth_lm[bidx + wlm1 + 1];

                weightSums_l[x + y * wl] = weightSums_lm[bidx]
                        + weightSums_lm[bidx + 1] + weightSums_lm[bidx + wlm1]
        ...
        AffLight aff_g2l)
{
    ...
    __m128 a = _mm_set1_ps(
            (float) (AffLight::fromToVecExposure(lastRef->ab_exposure,
    ...
    for (int i = 0; i < n; i += 4)
    {
        ...
        acc.updateSSE_eighted(_mm_mul_ps(id, dx), _mm_mul_ps(id, dy),
                _mm_sub_ps(zero,
                        _mm_mul_ps(id,
                                _mm_add_ps(_mm_mul_ps(u, dx),
                                        _mm_mul_ps(v, dy)))),
                _mm_sub_ps(zero,
                        _mm_add_ps(_mm_mul_ps(_mm_mul_ps(u, v), dx),
                                _mm_mul_ps(dy,
                                        _mm_add_ps(one, _mm_mul_ps(v, v))))),
                ...
                _mm_mul_ps(a,
                        _mm_sub_ps(b0, _mm_load_ps(buf_warped_refColor + i))),
        ...
        float cutoffTH)
{
    ...
    for (int i = 0; i < nl; i++)
    {
        ...
        if (lvl == 0 && i % 32 == 0)
        {
            ...
            sumSquaredShiftT += (KuT2 - x) * (KuT2 - x)
                    + (KvT2 - y) * (KvT2 - y);
...
bool CoarseTracker::trackNewestCoarse(FrameHessian* newFrameHessian,
        SE3 &lastToNew_out, AffLight &aff_g2l_out, int coarsestLvl,
        Vec5 minResForAbort, IOWrap::Output3DWrapper* wrap)
{
    ...
    for (int lvl = coarsestLvl; lvl >= 0; lvl--)
    {
        ...
        for (int iteration = 0; iteration < maxIterations[lvl]; iteration++)
        {
            ...
            {
                inc.head<6>() = Hl.topLeftCorner<6, 6>().ldlt().solve(
                        -b.head<6>());
                inc.tail<2>().setZero();
            }
            if (!(setting_affineOptModeA < 0) && setting_affineOptModeB < 0)// fix b
            {
                inc.head<7>() = Hl.topLeftCorner<7, 7>().ldlt().solve(
                        -b.head<7>());
                inc.tail<1>().setZero();
            }
            ...
            SE3 refToNew_new = SE3::exp((Vec6) (incScaled.head<6>()))
                    * refToNew_current;
...
CoarseDistanceMap::CoarseDistanceMap(int ww, int hh)
{
    ...
    coarseProjectionGrid = new PointFrameResidual*[2048
            * (ww * hh / (fac * fac))];
