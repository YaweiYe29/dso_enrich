...
Vec4 FullSystem::trackNewCoarse(FrameHessian* fh)
{
    ...
    else
    {
        ...
        for (float rotDelta = 0.02; rotDelta < 0.05; rotDelta++)
        {
            ...
            lastF_2_fh_tries.push_back(
                    fh_2_slast.inverse() * lastF_2_slast
                            * SE3(
                                    Sophus::Quaterniond(1, -rotDelta, rotDelta,
                                            0), Vec3(0, 0, 0))); // assume constant motion.
            lastF_2_fh_tries.push_back(
                    fh_2_slast.inverse() * lastF_2_slast
                            * SE3(
                                    Sophus::Quaterniond(1, 0, -rotDelta,
                                            rotDelta), Vec3(0, 0, 0)));	// assume constant motion.
            lastF_2_fh_tries.push_back(
                    fh_2_slast.inverse() * lastF_2_slast
                            * SE3(
                                    Sophus::Quaterniond(1, -rotDelta, 0,
                                            rotDelta), Vec3(0, 0, 0)));	// assume constant motion.
            lastF_2_fh_tries.push_back(
                    fh_2_slast.inverse() * lastF_2_slast
                            * SE3(
    ...
    for (unsigned int i = 0; i < lastF_2_fh_tries.size(); i++)
    {
        ...
        bool trackingIsGood = coarseTracker->trackNewestCoarse(fh,
                lastF_2_fh_this, aff_g2l_this, pyrLevelsUsed - 1, achievedRes);	// in each level has to be at least as good as the last try.
        tryIterations++;
    ...
    for (FrameHessian* host : frameHessians)	// go through all active frames
    {
        ...
        for (unsigned int i = 0; i < host->immaturePoints.size(); i += 1)
        {
            ...
            if ((u > 0 && v > 0 && u < wG[1] && v < hG[1]))
            {
                ...
                float dist = coarseDistanceMap->fwdWarpedIDDistFinal[u
                        + wG[1] * v] + (ptp[0] - floorf((float) (ptp[0])));
    ...
    if (multiThreading)
        treadReduce.reduce(
                boost::bind(&FullSystem::activatePointsMT_Reductor, this,
                        &optimized, &toOptimize, _1, _2, _3, _4), 0,
                toOptimize.size(), 50);
...
void FullSystem::makeNewTraces(FrameHessian* newFrame, float* gtDepth)
{
    ...
    for (int y = patternPadding + 1; y < hG[0] - patternPadding - 2; y++)
        for (int x = patternPadding + 1; x < wG[0] - patternPadding - 2; x++)
        {
            ...
            ImmaturePoint* impt = new ImmaturePoint(x, y, newFrame,
                    selectionMap[i], &Hcalib);
...
void FullSystem::printLogLine()
{
    ...
    if (!setting_debugout_runquiet)
        printf(
                "LOG %d: %.3f fine. Res: %d A, %d L, %d M; (%'d / %'d) forceDrop. a=%f, b=%f. Window %d (%d)\n",
