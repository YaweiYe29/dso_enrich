...
PhotometricUndistorter::PhotometricUndistorter(std::string file,
        std::string noiseImage, std::string vignetteImage, int w_, int h_)
{
    valid = false;
    vignetteMap = 0;
    vignetteMapInv = 0;
...

    {
        ...
        {
            printf(
                    "PhotometricUndistorter: invalid format! got %d entries in first line, expected at least 256!\n",
                    (int) Gvec.size());
            return;
        }
        ...
        for (int i = 0; i < GDepth - 1; i++)
        {
            ...
            {
                printf(
                        "PhotometricUndistorter: G invalid! it has to be strictly increasing, but it isnt!\n");
                return;
            }
        }
    ...
    if (vm16 != 0)
    {
        ...
        {
            printf(
                    "PhotometricUndistorter: Invalid vignette image size! got %d x %d, expected %d x %d\n",
                    vm16->w, vm16->h, w, h);
            if (vm16 != 0)
                delete vm16;
            if (vm8 != 0)
        ...
        std::string gammaFilename, std::string vignetteFilename)
{
            ...
            == 8)
    {
        ...
        if (!u->isValid())
        {
            ...
            &ic[1], &ic[2], &ic[3], &ic[4], &ic[5], &ic[6], &ic[7]) == 8)
    {
...
{
    photometricUndist = new PhotometricUndistorter(file, noiseImage,
            vignetteImage, getOriginalSize()[0], getOriginalSize()[1]);
}
        ...
        float exposure, double timestamp, float factor) const
{
    ...
    if (!passthrough)
    {
        ...
        if (benchmark_varNoise > 0)
        {
            ...
            {
                noiseMapX[i] = 2 * benchmark_varNoise
                        * (rand() / (float) RAND_MAX - 0.5f);
                noiseMapY[i] = 2 * benchmark_varNoise
                        * (rand() / (float) RAND_MAX - 0.5f);
            }
        }
...
void Undistort::makeOptimalK_crop()
{
    ...
    while (oobLeft || oobRight || oobTop || oobBottom)
    {
        ...
        for (int y = 0; y < h; y++)
        {
            remapX[y * 2] = minX;
            remapX[y * 2 + 1] = maxX;
            remapY[y * 2] = remapY[y * 2 + 1] = minY
                    + (maxY - minY) * (float) y / ((float) h - 1.0f);
        }
        ...
        for (int x = 0; x < w; x++)
        {
            remapY[x * 2] = minY;
            remapY[x * 2 + 1] = maxY;
            remapX[x * 2] = remapX[x * 2 + 1] = minX
                    + (maxX - minX) * (float) x / ((float) w - 1.0f);
        }
        ...
        {
            printf(
                    "FAILED TO COMPUTE GOOD CAMERA MATRIX - SOMETHING IS SERIOUSLY WRONG. ABORTING \n");
            exit(1);
        }
    }
        ...
        std::string prefix)
{
    ...
    } else if (nPars == 8) // KB, equi & radtan model
    {
        ...
        if (std::sscanf(l1.c_str(), buf, &parsOrg[0], &parsOrg[1], &parsOrg[2],
                &parsOrg[3], &parsOrg[4], &parsOrg[5], &parsOrg[6], &parsOrg[7])
                == 8 && std::sscanf(l2.c_str(), "%d %d", &wOrg, &hOrg) == 2)
        {
            printf("Input resolution: %d %d\n", wOrg, hOrg);
    ...
    {
        printf(
                "called with invalid number of parameters.... forgot to implement me?\n");
        infile.close();
        return;
    }
        ...
        float* out_y, int n) const
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        const float r = theta + k0 * theta3 + k1 * theta5 + k2 * theta7
                + k3 * theta9;
