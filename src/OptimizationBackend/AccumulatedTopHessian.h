...
public:
    ...
    template<int mode> void addPoint(EFPoint* p,
        ...
        if (MT)
        {
            ...
            red->reduce(
                    boost::bind(&AccumulatedTopHessianSSE::stitchDoubleInternal,
                            this, Hs, bs, EF, usePrior, _1, _2, _3, _4), 0,
    ...
    EIGEN_ALIGN16
    AccumulatorApprox* acc[NUM_THREADS];

    int nres[NUM_THREADS];

    template<int mode> void addPointsInternal(std::vector<EFPoint*>* points,
            EnergyFunctional const * const ef, int min = 0, int max = 1,
