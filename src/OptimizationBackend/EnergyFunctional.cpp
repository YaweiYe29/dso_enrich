        ...
        for (int t = 0; t < nFrames; t++)
        {
            ...
            Vec2f affLL =
                    AffLight::fromToVecExposure(host->ab_exposure,
                            target->ab_exposure, host->aff_g2l_0(),
...
void EnergyFunctional::setDeltaF(CalibHessian* HCalib)
{
    ...
    for (int h = 0; h < nFrames; h++)
        for (int t = 0; t < nFrames; t++)
        {
            int idx = h + t * nFrames;
            adHTdeltaF[idx] =
                    frames[h]->data->get_state_minus_stateZero().head<8>().cast<
                            float>().transpose() * adHostF[idx]
                            + frames[t]->data->get_state_minus_stateZero().head<
                                    8>().cast<float>().transpose()
                                    * adTargetF[idx];
        }
...
void EnergyFunctional::accumulateAF_MT(MatXX &H, VecX &b, bool MT)
{
    ...
    {
        red->reduce(
                boost::bind(&AccumulatedTopHessianSSE::setZero, accSSE_top_A,
                        nFrames, _1, _2, _3, _4), 0, 0, 0);
        red->reduce(
                boost::bind(&AccumulatedTopHessianSSE::addPointsInternal<0>,
                        accSSE_top_A, &allPoints, this, _1, _2, _3, _4), 0,
                allPoints.size(), 50);
...
void EnergyFunctional::accumulateLF_MT(MatXX &H, VecX &b, bool MT)
{
    ...
    {
        red->reduce(
                boost::bind(&AccumulatedTopHessianSSE::setZero, accSSE_top_L,
                        nFrames, _1, _2, _3, _4), 0, 0, 0);
        red->reduce(
                boost::bind(&AccumulatedTopHessianSSE::addPointsInternal<1>,
                        accSSE_top_L, &allPoints, this, _1, _2, _3, _4), 0,
                allPoints.size(), 50);
...
void EnergyFunctional::accumulateSCF_MT(MatXX &H, VecX &b, bool MT)
{
    ...
    {
        red->reduce(
                boost::bind(&AccumulatedSCHessianSSE::setZero, accSSE_bot,
                        nFrames, _1, _2, _3, _4), 0, 0, 0);
        red->reduce(
                boost::bind(&AccumulatedSCHessianSSE::addPointsInternal,
                        accSSE_bot, &allPoints, true, _1, _2, _3, _4), 0,
                allPoints.size(), 50);
...
void EnergyFunctional::resubstituteF_MT(VecX x, CalibHessian* HCalib, bool MT)
{
    ...
    for (EFFrame* h : frames)
    {
        ...
        for (EFFrame* t : frames)
            xAd[nFrames * h->idx + t->idx] = xF.segment < 8
                    > (CPARS + 8 * h->idx).transpose()
                            * adHostF[h->idx + nFrames * t->idx] + xF.segment
                    < 8
                    > (CPARS + 8 * t->idx).transpose()
                            * adTargetF[h->idx + nFrames * t->idx];
    }

    if (MT)
        red->reduce(
                boost::bind(&EnergyFunctional::resubstituteFPt, this, cstep,
                        xAd, _1, _2, _3, _4), 0, allPoints.size(), 50);
    else
            ...
            for (int i = 0; i + 3 < patternNum; i += 4)
            {
                ...
                Jdelta = _mm_add_ps(Jdelta,
                        _mm_mul_ps(_mm_load_ps(((float*) (rJ->JIdx + 1)) + i),
                                Jp_delta_y));
                Jdelta = _mm_add_ps(Jdelta,
                        _mm_mul_ps(_mm_load_ps(((float*) (rJ->JabF)) + i),
                                delta_a));
                Jdelta = _mm_add_ps(Jdelta,
...
EFResidual* EnergyFunctional::insertResidual(PointFrameResidual* r)
{
    ...
    connectivityMap[(((long) efr->host->frameID) << 32)
            + ((long) efr->target->frameID)][0]++;
...
EFFrame* EnergyFunctional::insertFrame(FrameHessian* fh, CalibHessian* Hcalib)
{
    ...
    {
        connectivityMap[(((long) eff->frameID) << 32) + ((long) fh2->frameID)] =
                Eigen::Vector2i(0, 0);
        if (fh2 != eff)
            connectivityMap[(((long) fh2->frameID) << 32)
                    + ((long) eff->frameID)] = Eigen::Vector2i(0, 0);
    }
...
void EnergyFunctional::dropResidual(EFResidual* r)
{
    ...
    connectivityMap[(((long) r->host->frameID) << 32)
            + ((long) r->target->frameID)][0]--;
    nResiduals--;
    r->data->efResidual = 0;
    delete r;
}
...
//	std::cout << std::setprecision(16) << "SVecI: " << SVecI.transpose() << "\n\n";
    ...
    HMScaled.topLeftCorner(ndim, ndim).noalias() -= bli
    ...
    // set.
    HM = 0.5
            * (HMScaled.topLeftCorner(ndim, ndim)
        ...
        CalibHessian* HCalib)
{
    ...
    if (setting_solverMode & SOLVER_SVD)
    {
        ...
        MatXX HFinalScaled = SVecI.asDiagonal() * HFinal_top
                * SVecI.asDiagonal();
    ...
    {
        VecX SVecI =
                (HFinal_top.diagonal() + VecX::Constant(HFinal_top.cols(), 10)).cwiseSqrt().cwiseInverse();
        MatXX HFinalScaled = SVecI.asDiagonal() * HFinal_top
                * SVecI.asDiagonal();
        x = SVecI.asDiagonal()
